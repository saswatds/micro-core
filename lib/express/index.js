class Express {
    constructor(config, logger) {
        ...
        this._errorHandler = (err, req, res, next) => {
            if (err.name !== 'Error') {
                // If status is not found then error is thrown by some application because of some conflict, like validation error
                return res.status(err.status|| 409).json({ code: err.status || 409, message: err.message })
            }
            if(err.response !== null) {
                return res.status(err.response.status || 500).json(err.response.text)
            }
    ...
    prepare() {
        ...
        if (this.config.healthCheck) {
            this.app.get('/_health', (req, res) => { res.json({ STATUS: 'OK' }) })
        }
